...
const DEV_TSCONFIG_NAME = "tsconfig.json";
const GAME_TSCONFIG_REL = path.join("apps", "web", "tsconfig.game.json");
const DEV_TSCONFIG_SEARCH_DEPTH = 6;

/**
 * Locate the monorepo's apps/web/tsconfig.game.json by walking up from a
 * project directory. On developer machines the game lives at
 * `<monorepo>/data/<name>`, so the lookup succeeds; on data-only (non-developer)
 * machines the game is a standalone checkout and this returns null.
 */
function findGameTsconfig(projectPath) {
  let dir = path.resolve(projectPath);
  for (let i = 0; i <= DEV_TSCONFIG_SEARCH_DEPTH; i += 1) {
    if (existsSync(path.join(dir, GAME_TSCONFIG_REL))) {
      return path.join(dir, GAME_TSCONFIG_REL);
    }
...
/**
 * Write the IDE-only `tsconfig.json` that gives developers TypeScript/React
 * tooling for custom code in `src/`. It extends the monorepo's shared game
 * config via a relative path, so it is only valid inside the monorepo — the
 * file is gitignored in the game repo and regenerated here per machine. On
 * machines without the monorepo this is a no-op (and its absence is expected).
 * Never throws; returns whether the file was written.
 */
export async function ensureDevTsconfig(projectPath) {
  try {
    const gameTsconfig = findGameTsconfig(projectPath);
    if (!gameTsconfig) return false;
    let relative = path.relative(path.resolve(projectPath), gameTsconfig).split(path.sep).join("/");
...
`;
    const target = path.join(projectPath, DEV_TSCONFIG_NAME);
